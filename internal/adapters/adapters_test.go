...
func TestNeutralOutput(t *testing.T) {
	for _, tc := range []struct{ agent, raw, want string }{
		{"codex", `{"hook_event_name":"Stop"}`, `{"continue":true}`}, {"workbuddy", `{}`, `{"continue":true}`}, {"cursor", `{"hook_event_name":"beforeSubmitPrompt"}`, `{"continue":true}`}, {"cursor", `{"hook_event_name":"stop"}`, `{}`}, {"claude-code", `{}`, `{}`}, {"unknown", `{}`, `{}`},
	} {
