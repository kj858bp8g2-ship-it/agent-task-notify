...
Darwin privacy also requires no additional extended ACL entries; modes alone are insufficient. Use a thin libc/cgo bridge in `acl_darwin.go` to inspect `ACL_TYPE_EXTENDED`, release native allocations, and fail closed on unsupported/error results. Conservatively reject any nonempty ACL on an existing nonempty directory or target file, preserving it. Only a newly created or verified empty owned directory and newly created exclusive temporary files may have inherited ACLs cleared before data is written. Production must not invoke `chmod` or another helper executable. Add Darwin tests for inherited ACL removal on owned empty/new paths, rejection without ACL changes on nonempty existing directories, and private files after replacement; a test-only absolute `/bin/chmod` may set ACLs on synthetic fixtures. These tests and the cgo bridge must execute in Task 4; an earlier non-cgo cross-compilation does not validate the amended Darwin implementation. Follow Apple's actual `acl_get_entry` return semantics (success is zero), not Linux conventions.
...
Actual-CI follow-up: the filesec completeness regression was written on Windows and cannot be executed there. Before counting the repaired Darwin gate as complete, the macOS CI script must run `TestDarwinRejectsIncompleteFileSecurity` against the current test files plus the exact pre-fix `internal/store/acl_darwin.go` from commit `0062d3b1ccc08c2b81112d8c843b8800f3af4df2` in a separate generated directory under the existing private CI temporary root. Keep the real checkout unchanged. Fetch the repository history through the pinned checkout action (`fetch-depth: 0`) solely so this fixed historical public blob is available. The counterfactual is successful only when the test exits nonzero and its output contains both `stage=unfilled result=1` and the exact failure `incomplete filesec accepted or complete no-ACL filesec rejected`; a compile error, unexpected success or other failure must fail CI. After that, run the unchanged real-source full suite and require the new test's explicit PASS line as well as the locked-Keychain PASS. Bound the focused test with a 45-second test timeout; keep each existing clang/run 10-second limit and the full-suite budgets unchanged. Retain logs only within the generated fixture and emit fixed safe evidence markers, not raw paths/compiler output. Cleanup must cover the generated copy through the existing validated temporary root. Add focused source/orchestration regression checks in the existing `tests/native_ci_test.go`; local tests must never invoke real macOS security commands. This provides retrospective actual RED/GREEN evidence, not a claim that the original sequence had already run RED.
