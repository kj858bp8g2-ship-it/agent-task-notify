# Native compatibility — 0.2.0-rc.2

This candidate is not validated by historical legacy Codex/iPhone use. Windows x64 is the local test platform; Mac Intel/Apple Silicon, ntfy/Android, WorkBuddy, OpenClaw, and Hermes are experimental. No user-owned physical Mac, real OpenClaw/Hermes/WorkBuddy host, first Keychain authorization, Gatekeeper first launch, Android, or new iOS phone E2E has been tested for this candidate. Mac packages are UNSIGNED and NOT NOTARIZED: ordinary auto-install stops; explicit developer evaluation must respect OS security without bypasses.

## Separate evidence levels

The table describes the `0.2.0-rc.2` contracts. Fresh exact-source CI, canonical archive consumers, tag publication, and downloaded-asset verification are recorded separately in [validation](native-validation.md); any still-open gate is marked there. The released `0.2.0-rc.1` run and hashes remain historical evidence for that tag only and are not attributed to the new integrations or changed archive inventory.

| Source / ID | Notification icon | Implemented | Contract | System | Process | Package | Real host | Phone |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Codex `codex` | ChatGPT artwork | Yes | Fixture-tested | Local Windows pass; CI pending | Synthetic CLI/hook/worker passed | Local archive pass; CI pending | Untested | Untested |
| Claude Code `claude-code` | Claude artwork | Yes | Fixture-tested | Local Windows pass; CI pending | Synthetic CLI/hook/worker passed | Local archive pass; CI pending | Untested | Untested |
| Cursor `cursor` | Cursor artwork | Yes | Fixture-tested | Local Windows pass; CI pending | Synthetic CLI/hook/worker passed | Local archive pass; CI pending | Untested | Untested |
| Gemini CLI `gemini-cli` | Gemini artwork | Yes | Fixture-tested | Local Windows pass; CI pending | Synthetic CLI/hook/worker passed | Local archive pass; CI pending | Untested | Untested |
| OpenCode `opencode` | OpenCode artwork | Yes | JS/native bridge fixtures | Local Windows pass; CI pending | Synthetic bridge/CLI passed | Local archive pass; CI pending | Untested | Untested |
| WorkBuddy `workbuddy` | WorkBuddy artwork | Experimental manual package | Shared CodeBuddy shape only | Local Windows pass; CI pending | Synthetic packaged launcher passed | Local archive pass; CI pending | Untested | Untested |
| OpenClaw `openclaw` | OpenClaw artwork | Experimental manual plugin | Official typed-hook contract + JS fixtures | Local Windows pass; CI pending | Privacy-minimized direct spawn passed | Local archive pass; CI pending | Untested | Untested |
| Hermes Agent `hermes` | Hermes artwork | Experimental manual Shell Hooks | Official wire contract + Go fixtures | Local Windows pass; CI pending | Synthetic hook payloads passed | Local archive pass; CI pending | Untested | Untested |

System means native filesystem/protected-storage tests on Windows and four actual macOS CI runners, not physical-device authorization. Process means isolated child execution with synthetic inputs/local fake services, not real host E2E. Package means actual downloaded canonical-archive verification and dry CLI execution, not live host loading or phone delivery. Mac source package tests also exercised the literal-backslash WorkBuddy launcher and revised unsigned notice. In guarded Mac protected-process tests, inherited CI HOME is retained only after both exact child/inherited Keychain contexts are proven to contain the dedicated disposable fixture; app data/config/CWD remain synthetic. Other readonly/absent/unsafe cases use synthetic HOME. This is not proof of protected writes under arbitrary new HOME. Mac Security.framework and pinned download-action runtime/deprecation warnings remain; some foreign-owner cases skip without available privilege. Locked-Keychain denial passed explicitly on all four Mac runners, not as a skip. Task terminal state/released lock proves logical completion, not hard OS-process exit or containment.
...
| `openclaw` | Manual complete `openclaw` plugin; packaged direct-spawn runtime; explicit conversation-hook permission and Gateway restart | [OpenClaw typed plugin hooks](https://github.com/openclaw/openclaw/blob/main/docs/plugins/hooks.md), checked 2026-08-31; no real host |
| `hermes` | Manual complete `hermes` Shell Hooks package; merge two commands into `~/.hermes/config.yaml`; per-command consent | [Hermes Event Hooks](https://hermes-agent.nousresearch.com/docs/user-guide/features/hooks), checked 2026-08-31; no real host |

`--config-path` is a verified explicit locator, not permission to guess desktop files. Host/plugin parents must already exist, be owned and validated; the installer never creates/repairs them. Existing empty/whitespace/malformed JSON is left untouched. Host JSON size limit is 4 MiB, backup envelope 6 MiB; unknown number lexemes/other hooks are preserved. A receipt records ownership, not loading, host trust or absence of another plugin route. Never double-register native and legacy paths. WorkBuddy, OpenClaw, and Hermes import their full self-contained subfolders and intentionally have no automatic host-config mutation or receipt-owned uninstall.
